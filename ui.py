...
try:
    from .function import AdbClient, ScreenshotSelector, Settings, resolve_path, _exe_dir
...
class _GlowLabel(tk.Label):
    def __init__(self, master, text="", color=NEON_CYAN, **kw):
        super().__init__(master, text=text, bg=BG_DARK, fg=color, font=FONT_BOLD, **kw)


class _Section(tk.Frame):
    """Dark LabelFrame với neon header line."""
    def __init__(self, parent, title="", accent=NEON_CYAN, **kw):
        ...
        tk.Label(hdr, text=title, bg=BG_CARD, fg=accent, font=FONT_BOLD).pack(side="left")
...
class App(tk.Frame):
    def __init__(self, master: tk.Tk):
        ...
        # setting.json nằm cạnh .exe (hoặc script)
        self.settings = Settings(os.path.join(_exe_dir(), "setting.json"))
        adb_path = resolve_path(self.settings.get("adb", "adb_path", default="ADB/adb.exe"))
    ...
    def _repo_root(self) -> str:
        """Thư mục gốc: khi frozen = thư mục chứa .exe; khi dev = thư mục script."""
        return _exe_dir()
    ...
    def _short_template_value(self, any_path: str) -> str:
        """Chuẩn hóa đường dẫn template về dạng './templates/<file>'."""
        p = (any_path or "").strip()
        if not p:
            return ""
        p_norm = p.replace("\\", "/")
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(bar, textvariable=self.var_status, bg=BG_DARK, fg=FG_DIM, font=FONT_SMALL).grid(row=1, column=0, sticky="w")
        tk.Label(bar, text="AutoHunt v1.0", bg=BG_DARK, fg=FG_DIM, font=FONT_SMALL).grid(row=1, column=1, sticky="e")
    ...
    def _show_tab(self, key: str) -> None:
        for k, frm in self._tab_frames.items():
            frm.grid_remove()
            self._tab_btns[k].config(bg=BG_PANEL, fg=FG_DIM, highlightthickness=0)
        self._tab_frames[key].grid(row=0, column=0, sticky="nsew")
        self._tab_btns[key].config(bg=BG_CARD, fg=NEON_CYAN,
    ...
    def _build_tab_main(self) -> None:
        ...
        self.cbo_serial = ttk.Combobox(g, textvariable=self.var_serial, state="readonly", font=FONT_UI)
        ...
        tk.Label(up_card, text="⏱ Thời gian hoạt động", bg=BG_INPUT, fg=FG_DIM, font=FONT_SMALL).pack()
        ...
        self.btn_start = _btn(st, "▶ START", self.on_start, accent=NEON_GREEN)
        self.btn_stop  = _btn(st, "■ STOP",  self.on_stop,  accent=NEON_PINK)
        self.btn_stop.config(state="disabled")
        self.btn_start.grid(row=2, column=0, sticky="ew", padx=(0, 3))
        self.btn_stop.grid( row=2, column=1, sticky="ew", padx=(3, 0))

        _btn(st, "🗑 Clear log", self.on_clear_log, accent=FG_DIM, small=True).grid(
    ...
    def _set_adb_busy(self, busy: bool, status_text: Optional[str] = None) -> None:
        ...
        try:
            ...
            pass
        if status_text:
            color = (NEON_YEL if busy
                     else (NEON_GREEN if "kết nối" in status_text.lower() and "đang" not in status_text.lower()
                           else FG_DIM))
    ...
    def _build_setting_coords(self, parent: tk.Frame) -> None:
        ...
        for c, h in enumerate(["Tên", "X", "Y", ""]):
            tk.Label(g, text=h, bg=BG_CARD, fg=FG_DIM, font=FONT_SMALL).grid(
                row=0, column=c, sticky="w", padx=(0, 8), pady=(0, 4))
    ...
    def _build_tab_info(self) -> None:
        ...
        # Divider
        tk.Frame(page, bg=BORDER_GLOW, height=1).grid(row=4, column=0, sticky="ew", padx=12, pady=(10, 4))
        ...
        for i, (col, num, title, desc) in enumerate(steps):
            card = tk.Frame(page, bg=BG_CARD, highlightthickness=1, highlightbackground=col)
            card.grid(row=6 + i, column=0, sticky="ew", padx=12, pady=3)
            card.columnconfigure(1, weight=1)
            badge = tk.Frame(card, bg=col, width=36)
            badge.grid(row=0, column=0, sticky="ns")
        ...
        # Lưu ý
        tk.Frame(page, bg=BORDER_GLOW, height=1).grid(row=11, column=0, sticky="ew", padx=12, pady=(12, 4))
        ...
        notes = [
            ("🖥️", "Chỉnh Display giả lập 1920x1080 (DPI 280) để có thể sử dụng setting mặc định"),
        ]
    ...
    def _load_settings_to_ui(self) -> None:
        ...
        for k, v in self.hold_vars.items():
            v.set(str(self.settings.get("hold_ms", k,
                       default=self.settings.get("hold_ms", "default", default=60))))
    ...
    def _save_ui_to_settings(self) -> None:
        ...
        for name, v in self.thr_vars.items():
            ...
            if raw == "":
                try:
                    if isinstance(self.settings.data.get("thresholds"), dict) \
                            and name in self.settings.data["thresholds"]:
    ...
    def on_find_emulator(self) -> None:
        ...
        ports = self.settings.get("adb", "scan_ports",
                                   default=[5555, 5556, 5557, 5558, 5560, 5575, 5585])
    ...
    def on_start(self) -> None:
        ...
        self.bot_thread = threading.Thread(
            target=hunter.run, args=(serial, self.stop_event), daemon=True
        )
