...
class AutoHunter:
    def __init__(
        ...
        adb_path = resolve_path(self.s.get("adb", "adb_path", default="AutoHuntDino/ADB/adb.exe"))
    ...
    def _after_map_switch(self, stop_event) -> bool:
        # Give the game time to load the next map before restarting Step 1.
    ...
    def _check_x_and_close(self, serial: str, stop_event) -> bool:
        # Step 1 partial: loop close X in ROI "check_x"
        x_tmpl = self._tmpl("X")
        if not x_tmpl:
            return False
    ...
    def _step2_pick_dino(self, serial: str, stop_event) -> Optional[Point]:
        # returns clicked dino center if picked
        rx1, ry1, rx2, ry2 = self._roi("vung_san")
        if rx1 == 0 and ry1 == 0 and rx2 == 0 and ry2 == 0:
            self.log("ROI 'vùng săn' đang = (0,0,0,0) -> tool sẽ không thể quét dino. Hãy vào Setting và 'Chụp & vẽ ROI'.")
        ...
        for attempt in range(1, max_scan + 1):
            ...
            if found:
                ...
                if r.center and self._tap(serial, r.center, "click_dino", stop_event):
                    return None
                return r.center
            # Diagnostic: show best scores to help tune threshold/templates.
            ...
            t2 = self._thr("Dino2")
            self.log(
                f"Không thấy dino (lần {attempt}/{max_scan}) | score Dino1={s1:.2f} (thr {t1:.2f}), Dino2={s2:.2f} (thr {t2:.2f})"
            )
    ...
    def _step4_hunt_and_check_success(self, serial: str, stop_event) -> bool:
        # click coord "hunt" then check X. if no X => success
    ...
    def run(self, serial: str, stop_event) -> None:
        ...
        while not stop_event.is_set():
            ...
            if success:
                ...
                # Step 5
                need_step5 = self.stats.success_since_step5 >= int(self.s.get("logic", "success_before_step5", default=4))
                if need_step5:
                    self.stats.success_since_step5 = 0
                    self.stats.step5_runs += 1
                    self.on_stats(self.stats)
                    self.log(f"Step5: đảo map (lần {self.stats.step5_runs}/{int(self.s.get('logic','step5_before_step6',default=5))})")
                    self._tap(serial, self._coord("dao_map"), "click_map", stop_event)
                ...
                # Important: after Step5/Step6 we must restart at Step 1.
