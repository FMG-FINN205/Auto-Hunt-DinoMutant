...
# ═══════════════════════════════════════════════════════════════════════════════
#  Tiện ích đường dẫn  (hoạt động cả khi chạy .py lẫn .exe do PyInstaller)
# ═══════════════════════════════════════════════════════════════════════════════

def _exe_dir() -> str:
    """Thư mục chứa file .exe (khi frozen) hoặc thư mục chứa script (khi dev)."""
...
def resolve_path(p: str) -> str:
    """
    Trả về đường dẫn tuyệt đối — hoạt động cả khi chạy .exe (PyInstaller).

    Quy tắc:
      • Đường dẫn tuyệt đối  → trả về nguyên.
      • Tiền tố 'AutoHuntDino/' bị strip để đường dẫn tương đối
        với thư mục chứa .exe / script.
      • Các đường dẫn còn lại join với _exe_dir().
    """
    if not p:
        return p
    p = p.replace("/", os.sep)
    ...
    tail = p[len(prefix):] if p.startswith(prefix) else p
    return os.path.abspath(os.path.join(_exe_dir(), tail))
...
def safe_sleep(stop_event, seconds: float) -> bool:
    """Ngủ có thể bị ngắt. Trả về True nếu stop_event được set."""
    if seconds <= 0:
...
class AdbClient:
    ...
    def _resolve_adb_path(adb_path: str) -> str:
        # 1. Đường dẫn người dùng cấu hình (đã resolve)
        if adb_path:
            p = resolve_path(str(adb_path))
            if os.path.exists(p):
                return p

        # 2. adb có trong PATH hệ thống
        p2 = shutil.which("adb")
        if p2 and os.path.exists(p2):
            return p2

        # 3. Thư mục ADB cạnh .exe / script
        base = _exe_dir()
        adb_name = "adb.exe" if os.name == "nt" else "adb"
        candidates = [
            os.path.join(base, "ADB", adb_name),
            os.path.join(os.path.dirname(base), "ADB", adb_name),
        ]
        for c in candidates:
            if os.path.exists(c):
                return c

        # Giữ nguyên để thông báo lỗi rõ đường dẫn đã thử
    ...
    def _run(self, args: List[str], timeout_s: float = 10.0) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                "Không tìm thấy adb. Hãy đảm bảo thư mục ADB/ nằm cạnh file .exe "
                "và chứa adb.exe.\n"
                f"Đường dẫn đang dùng: {self.adb_path}"
            ) from e
